...
func TestInstallModule(t *testing.T) {
	settings := cli.New()
	err := InstallModule(Module{Name: "cow", Alias: "mycow", Url: "https://github.com/crossett/altima-modules/releases/download/v0.0.1/cow-v0.0.1.tgz"}, settings.ModulesDir)
	if err != nil {
		t.Error("Could not install module!")
	}

	err = InstallModule(Module{Name: "cow", Url: "https://github.com/crossett/altima-modules/releases/download/v0.0.1/cow-v0.0.1.tgz"}, settings.ModulesDir)
	if err != nil {
		t.Error("Could not install module!")
	}
}
